...
CreateMSAction <- function(type, level, time, time_string, open, high, low, close) {
...
Run <-function() {
  ...
  while(TRUE) {
    ...
    if (length(tlv_list) == 9) {
      out <- CreateMSAction(tlv_list[[2]]$v, tlv_list[[3]]$v, tlv_list[[4]]$v, tlv_list[[5]]$v, tlv_list[[6]]$v, tlv_list[[7]]$v, tlv_list[[8]]$v, tlv_list[[9]]$v)
      print(out)
