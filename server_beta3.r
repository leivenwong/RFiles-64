...
Run <-function() {
  ...
  while(TRUE) {
    ...
    if (length(tlv_list) == 9) {
      out <- CreateMSAction(con1, tlv_list[[2]]$v, tlv_list[[3]]$v, tlv_list[[4]]$v, tlv_list[[5]]$v, tlv_list[[6]]$v, tlv_list[[7]]$v, tlv_list[[8]]$v, tlv_list[[9]]$v)
      print(out)
      #WriteTLVLIST(con1, out)
    } else if (length(tlv_list) == 1) {
      if (tlv_list[[1]]$v == "QUITR") {
        print("ctp ask to quit")
        break
      } else if (tlv_list[[1]]$v == "rlang-heartbeat") {
        #do nothing
      } else {
        print("not enough or too much param 2")
      }
    } else if (length(tlv_list) == 3) {
      if (tlv_list[[1]]$v == "Order.Stop") {
        WangFunctionStop(tlv_list[[2]]$v, tlv_list[[3]]$v, tlv_list[[4]]$v)
      }  else if (tlv_list[[1]]$v == "Order.StopWin") {
        WangFunctionStopWin(tlv_list[[2]]$v, tlv_list[[3]]$v, tlv_list[[4]]$v)
      }		
    } else {
